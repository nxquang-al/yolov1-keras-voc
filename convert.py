...
def _main(args):
    ...
    assert output_path.endswith(".hdf5"), "output path {} is not a .hdf5 file".format(
        output_path
    )
        ...
        if section.startswith("convolutional"):
            ...
            if batch_normalize:
                conv_layer = BatchNormalization(
                    weights=bn_weight_list, name="bn" + format(section)
                )(conv_layer)
            ...
            all_layers.append(
                MaxPooling2D(
                    padding="same", pool_size=(size, size), strides=(stride, stride)
                )(prev_layer)
            )
            ...
            reshape_layer = Reshape((side, side, classes + num * (coords + rescore)))(
                prev_layer
            )
            ...
            or section.startswith("crop")
            or section.startswith("detection")
